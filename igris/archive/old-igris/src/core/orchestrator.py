...
class IgrisOrchestrator:
    ...
    def run_loop(self, router_fn=None, interval_s: int = DEFAULT_LOOP_INTERVAL_S) -> None:
        ...
        print(f"Igris Orchestrator running. Loop interval: {interval_s}s")
        print(f"Active Idle triggers after {ACTIVE_IDLE_THRESHOLD_S // 60} min of inactivity.")
        print(f"Agents loaded: {len(self.agents)} | Tasks loaded: {len(self.tasks)}")
        print("─" * 50)
        ...
        try:
            while True:
                ...
                if router_fn:
                    # Build observation for the 8B router
                    agents_list = [
                        {
                            "agent_id": a.agent_id,
                            "rank": a.rank.value,
                    ...
                    # Simulation: auto-promote eligible agents, no-op otherwise
            ...
            self._save_state()
            print("\nIgris shut down. State saved.")

    def _simulate_router(self) -> str:
        """Simulate router decisions when no LLM is connected.

        Auto-promotes eligible agents. Spawns a new agent if none exist.
        """
        ...
        if len(self.agents) == 0:
            return json.dumps({
                ...
                "language": "python",
                "reason": "seed agent — no agents in registry",
            })
        ...
        return json.dumps({"action": "noop", "reason": "nothing to do"})
